...
//new imports that i added Colin Tran
...
//Fixed this function Colin Tran
@Override
public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
    if (call.method.equals("getPlatformVersion")) {
        ...
        connectClover(ip, port, result);
        
        //} else if (call.method.equals("showPaymentAmountOptions")) {
        //showPaymentAmountOptions(result);
    } else {
        result.notImplemented();
    }
}

//Added this function Colin Tran
private void connectClover(String ip, Integer port, Result result) {
    try {
        CloverDeviceConfiguration config = getNetworkConfiguration(ip, port);
        ICloverConnector connector = new CloverConnector(config);
        connector.initializeConnection();
        
    } catch (Exception e) {
        result.error("connection_error", "Failed to connect to Clover: " + e.getMessage(), null);
    }
}
  ...
  public CloverDeviceConfiguration getNetworkConfiguration(String ip, Integer port) {
    ...
    try {
      ...
      System.out.println("getNetworkConfiguration **** socket: ==> wss://" + ip + ":" + dvcPort + "/remote_pay");
          ...
          activity.runOnUiThread(new Runnable() {
            ...
            public void run() {
              ...
              if (pairingCodeDialog != null) {
                ...
                AlertDialog.Builder builder = new AlertDialog.Builder(activity); // I changed context to activity Colin Tran
